		...
		// Connect to a given device & uuid endpoint
			...
			try {
				...
				for( int i = 0; i < buffer.length; i++ ) {
					...
					sb2.append(Integer.toHexString(buffer[i]));
				...
				//for(int j = 0; j < buffer.length; j++)
            		//sb2.append(Integer.toHexString(buffer[j]));
				//Log.d( "BluetoothPlugin", "Buffer: " + String.valueOf(buffer) );
				StringBuilder sb1 = new StringBuilder("");
				sb1.append(String.valueOf(buffer));
				sb1.append("|");
				sb1.append(sb2.toString());
				pluginResult = new PluginResult(PluginResult.Status.OK, sb1.toString());
			}
			catch( Exception e ) {
				Log.e("BluetoothPlugin", e.toString() + " / " + e.getMessage() );
